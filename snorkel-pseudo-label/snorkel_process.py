...
def build_raw_data (filepath):
    ...
    for i in tqdm(range(len(allfile))):
        ...
        for j in range(len(temp)):
            ...
            allid.append(allfile.id.iloc[i])
    allsent=pd.DataFrame(allsent,columns=['sent'])
    allsent['pid']=allid
    return allfile, allsent

def loop_labing(keylist,valuelist):
    def keyword_lookup(x, keywords, label):
        if any(word in x.sent.lower() for word in keywords):
            return label
        return Norelevent
    ...
    for i in range(len(keylist)):
        ...
        globals()[vbname] = make_keyword_lf(keywords=valuelist[i],name=vbnameab,label=labelvalue)
